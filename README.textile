...
IK 1.0 only supports non-consumable purchases. Future releases aim to include support for consumables and subscriptions. 2.0 is in active development (master) and will include consumables (only stored on the device, not available on other platforms) and auto-renewable subscriptions (synced with third party service, stored locally for offline support).
